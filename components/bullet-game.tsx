...
// Simplified from map-runner.tsx — no map flying, no route sequence
"use client"
...
const WAVE_DURATION = 25   // seconds for normal level
const BOSS_WAVE_DURATION = 35 // seconds for boss
const SPAWN_INTERVAL_BASE = 600 // ms between bullets
...
export function BulletGame({
  ...
  // Sync health/score/combo via refs for the game loop
  ...
  // ── 3-2-1 countdown, then start game loop ──────────────────
  useEffect(() => {
  ...
  // ── Main game loop (starts after countdown) ─────────────────
  useEffect(() => {
    if (countdown > 0) return
    ...
    const loop = () => {
      ...
      // Update bullets (top-to-bottom: y increases)
      const active: Bullet[] = []
      for (const bullet of bulletsRef.current) {
        ...
        if (bullet.y > rect.height + 60) continue // off screen
        active.push(bullet)
      }
  ...
  return (
    ...
    >
      ...
      {/* Boss dark overlay */}
      {isBoss && (
        <div className="pointer-events-none absolute inset-0 z-10 bg-red-950/40" />
      )}

      {/* Flash */}
      {flash && (
        <div className="pointer-events-none absolute inset-0 z-50" style={{ backgroundColor: flash }} />
      )}

      {/* Company name banner */}
      <div
        className="absolute inset-x-0 top-0 z-20 pt-16 text-center"
        style={{ fontFamily: "var(--font-pixel, monospace)" }}
      >
        <div className="text-xs" style={{ color: isBoss ? "#f97316" : "#ffe66d" }}>
          {isBoss ? "⚡ BOSS BATTLE" : "📍 LEVEL"}
        </div>
      ...
      {countdown > 0 && (
        <div className="absolute inset-0 z-40 flex items-center justify-center">
          <div
            className="text-8xl font-bold text-[#ffe66d]"
            style={{
              fontFamily: "var(--font-pixel, monospace)",
              textShadow: "0 0 40px rgba(255,230,100,0.8)",
      ...
      >
        ...
        {/* Player character (at bottom, moves left-right) */}
        <div
        ...
        >
          <div
            className="flex h-full w-full items-center justify-center rounded-lg border-2 text-2xl"
            style={{
              borderColor: isBoss ? "#f97316" : "#00ff88",
              background: isBoss ? "#f9731630" : "#00ff8830",
              boxShadow: isBoss ? "0 0 20px #f9731640" : "0 0 20px #00ff8840",
            }}
          >
            🏃
          </div>
